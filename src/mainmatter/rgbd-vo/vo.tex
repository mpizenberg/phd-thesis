...
\begin{itemize}
	\item 3D to 3D:\@ features are associated as 3D points.
		It requires knowing the depth of points in the image,
		which is facilitated by stereo or RGB-D cameras.
	\item 2D to 2D:\@ only the image coordinates of features are
		used to estimate the camera motion.
	\item 3D to 2D:\@ 3D points from previous frames
		are matched to 2D coordinates in the current frame.
\end{itemize}
