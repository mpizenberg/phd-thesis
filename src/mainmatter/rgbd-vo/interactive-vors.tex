...
difficulty of building annotation datasets is the required human annotation time.
Visual Odometry is quite different on this point.
Algorithms are strongly related to capture devices,
be it stereo, mono, RGB-D cameras, or cameras paired with other sensors
such as inertial measurement units (IMU), GPS or lidars.
As a consequence, there are many datasets focused on different acquisition systems.
...
The main difference between visual odometry and annotation is thus that
the difficulty is technological here, while it is mostly time consumption for annotation.

New College vision and laser dataset~\cite{smith2009new}.
robot with stereo images, omnidirectional images, lidar, gps, imu.

Vision meets robotics: the KITTI dataset~\cite{geiger2013vision}.
Gray stero camera, color stere camera, lidar (10 Hz), gps-IMU (100 Hz).

SemanticKITTI, semantic scene understanding using lidar sequences~\cite{behley2019iccv}.
Provide dense point cloud semantic annotation.
http://semantic-kitti.org/

A benchmark for the evaluation of RGB-D SLAM systems~\cite{sturm2012benchmark}.
RGB-D images (depth + color), imu.
Ground truth with 8 Raptor-E cameras,
motion capture software by MotionAnalysis~\cite{MotionAnalysis}.
...
Bonn RGB-D Dynamic Dataset~\cite{palazzolo2019iros}.
TUM RGB-D Dataset format.
Ground truth recorded with Optitrack Prime 13 motion capture system.
Ground truth 3D point cloud of static environment recorded with Leica BLK360 lidar.
The 3D surface is also provided for the living room scene.

Photometrically calibrated benchmark mono vo~\cite{engel2016photometrically}.
Color mono camera, photometrically calibrated (sensor response, vignette, exposure time).
Loop closed with starting and ending ground truth.

TUM VI benchmark~\cite{schubert2018tum}.

ADVIO dataset~\cite{cortes2018advio}.

EuRoC dataset~\cite{burri2016euroc}.

University of Michigan,
North Campus Long-Term (NCLT) Vision and Lidar dataset.~\cite{carlevaris2016university}.

\subsubsection{Motion Capture for RGB-D Visual Odometry}%
\label{ssub:motion_capture}

Parler un peu plus en détail du systeme de capture du dataset RGB-D cremers.
Qui est celui qui défini le standard utilisé par notre approche.

\subsubsection{Synthetic Dataset Creation}%
\label{ssub:synthetic_dataset}

Parler un peu plus du dataset ICL-NUIM.
